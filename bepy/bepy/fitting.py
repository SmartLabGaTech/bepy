...
def shoFit(
    ...
    chirps = chirpMap[chirpMap["PlotGroup"] != 9].index
...
def shoFitAcq(
    ...
    if exportPath is not None:
        ...
        raw_df = spec.loc[
            chirpMap[(chirpMap["PlotGroup"] != 9)].index[0] :,
            np.logical_and(
        ...
        export_df[mask.loc[(acqNum, chirpMap[(chirpMap["PlotGroup"] != 9)].index[0]) :].values] = (
            np.nan + 1j * np.nan
        )
...
#     ####Set chirps for reconstructed data index
#     chirps = (
#         chirpMap[chirpMap["PlotGroup"] != 9].index[:-1]
#     )
#     ####Set up columns
#     freq_cols = chirp_df(fftFileName, acqNum).columns
#     freq_cols = freq_cols[
...
#     #####Combined for future use
#     combined = real_data.values + (1j * im_data.values)
#     comb_df = pd.DataFrame(combined, index=chirps, columns=freq_cols)
#     #########Load Inital SHO results preBayes
#     sho_file = "shofit.csv"
#     sho_df = pd.read_csv(shoPath + sho_file, index_col=[0, 1]).loc[acqNum].iloc[:-1]
#     #####Generate Error Mask from initial results####
#     mask = find_outliers_Q(sho_df, 0.4)
#     ####Refit unreliable chirps
#     for chirp in sho_df[mask].index:
#         if chirp in keep_first:
#             continue
#         else:
#             new_vals = be.fitChirp(
#                 comb_df,
#                 lowFreq=low,
#                 highFreq=high,
#                 chirpMap=chirpMap,
...
#                 fittype=fittype,
#             )[-8:]
#             sho_df.loc[chirp, idx["Amp":"errQ"]] = new_vals
#     return sho_df
...
def BayeSMG_UQ(
    ...
    #####Extract a UQ copy for sho fitting
    im_file = imFile + str(copyNum) + ".csv"
    im_data = pd.read_csv(im_file, header=None)
    # ## Load completed Real data
    real_file = realFile + str(copyNum) + ".csv"
    real_data = pd.read_csv(real_file, header=None)
    #####Combined for future use
    combined = real_data.values + (1j * im_data.values)
    comb_df = pd.DataFrame(combined, index=chirps, columns=freq_cols)
    #####Generate Error Mask from initial results####
    mask = find_outliers_Q(sho_DF, errQ_Ratio)
    ####For Slicing
    idx = pd.IndexSlice

    ####Refit unreliable chirps
    for chirp in sho_DF[mask].index:
        if chirp in keepFirst:
            continue
        else:
            if initGuess is not None:
                tempGuess = []
                for copy in [1000, 999]:
                    tempGuess.append(initGuess.loc[copy, acqNum, chirp].values)
            else:
                tempGuess = None
